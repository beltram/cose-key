...
mod error;
pub use error::CoseKeyConfirmationError;
...
pub enum KeyConfirmation {
    CoseKey(crate::CoseKey) = Self::COSE_KEY_CLAIM,
    ...
    Thumbprint(crate::thumbprint::CoseKeyThumbprint<32>) = Self::THUMBPRINT_CLAIM,
}
    ...
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        impl<'de> serde::de::Visitor<'de> for KeyConfirmationVisitor {
            ...
            fn visit_map<A: serde::de::MapAccess<'de>>(self, mut map: A) -> Result<Self::Value, A::Error> {
                ...
                use coset::AsCborValue as _;
                Ok(match key {
                    Self::Value::COSE_KEY_CLAIM => {
                        Self::Value::CoseKey(map.next_value::<crate::CoseKey>().map_err(A::Error::custom)?)
                    }
                    Self::Value::ENCRYPTED_COSE_KEY_CLAIM => {
                        ...
                        Self::Value::EncryptedCoseKey(
                            if let Ok(to_recipient) = coset::CoseEncrypt::from_cbor_value(value.clone()) {
                    ...
                    Self::Value::KID_CLAIM => {
                        Self::Value::Kid(map.next_value::<serde_bytes::ByteBuf>().map_err(A::Error::custom)?)
                    }
                    #[cfg(feature = "thumbprint")]
                    Self::Value::THUMBPRINT_CLAIM => KeyConfirmation::Thumbprint(
                        map.next_value::<crate::thumbprint::CoseKeyThumbprint>()
                            .map_err(A::Error::custom)?,
                    ),
                    unknown => {
                        return Err(A::Error::custom(format!(
                            "Unknown KeyConfirmation discriminant: {unknown}",
                        )));
                    }
                })
            }
...
mod pem {
    ...
    impl KeyConfirmation {
        pub fn from_public_key_pem<K: pkcs8::DecodePublicKey + TryInto<CoseKey, Error = E>, E: Into<CoseKeyError>>(
            s: &str,
        ) -> Result<Self, error::CoseKeyConfirmationError> {
...
#[cfg(feature = "thumbprint")]
impl KeyConfirmation {
    pub fn new_thumbprint(
        pk: impl TryInto<crate::CoseKey, Error: Into<error::CoseKeyConfirmationError>>,
    ) -> Result<Self, error::CoseKeyConfirmationError> {
        let key = pk.try_into().map_err(Into::into)?;
        let thumbprint = crate::thumbprint::CoseKeyThumbprint::<32>::compute::<sha2::Sha256>(key)?;
...
mod tests {
    ...
    fn should_support_cose_key() {
        let (cose_key, x, y) = {
            ...
            let key = crate::CoseKey::from(key);
    ...
    fn should_support_thumbprint() {
        let (cose_key, expected_thumbprint) = {
            ...
            let key = coset::CoseKeyBuilder::new_ec2_pub_key(iana::EllipticCurve::P_256, x, y)
                .key_id(kid.clone())
                .build();
            (crate::CoseKey::from(key), Value::Bytes(kid))
        };
