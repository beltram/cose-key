...
macro_rules! thumbprint_compute {
    ($length:literal, $size:ty) => {
        impl CoseKeyThumbprint<$length> {
            ...
            pub fn compute<Hasher>(
                key: impl TryInto<crate::CoseKey, Error: Into<error::CoseKeyThumbprintError>>,
            ) -> Result<Self, error::CoseKeyThumbprintError>
            where
...
impl<const N: usize> CoseKeyThumbprint<N> {
    fn _compute(
        key: impl TryInto<crate::CoseKey, Error: Into<CoseKeyThumbprintError>>,
    ) -> Result<Vec<u8>, CoseKeyThumbprintError> {
        use coset::iana::{self, EnumI64 as _};

        let key: crate::CoseKey = key.try_into().map_err(Into::into)?;
...
-----END PUBLIC KEY-----"#;
        const EXPECTED_P256_PUBLIC_KEY_ENCODING: &str = r#"a4010220012158206cb288afc6c92f519e896d5cd6f3f894ea2c389a0c43d9b3323967f2ba5f5fa322582049f8fdb33b9a8c309bd1e084ed98014b3eae18ccbac0f65dcfb51b88bc353c00"#;
        const ED25519_PUBLIC_KEY: &str = r#"-----BEGIN PUBLIC KEY-----
MCowBQYDK2VwAyEAAPT6nOPM+sC2JTf5mjzxSA1noZJND75aQjWqw80LLcM=
-----END PUBLIC KEY-----"#;
        const EXPECTED_ED25519_PUBLIC_KEY_ENCODING: &str =
            r#"a30101200621582000f4fa9ce3ccfac0b62537f99a3cf1480d67a1924d0fbe5a4235aac3cd0b2dc3"#;

        let p256_key = p256::PublicKey::from_public_key_pem(P256_PUBLIC_KEY).unwrap();
        let thumbprint = CoseKeyThumbprint::<32>::_compute(p256_key)
            .unwrap()
            .encode_hex::<String>();
        ...
        let thumbprint = CoseKeyThumbprint::<32>::_compute(ed25519_key)
            .unwrap()
            .encode_hex::<String>();
