use crate::EcdsaCoseKeyExt;
use coset::iana;
use signature::{Keypair, KeypairRef};

mod error;

pub use error::CoseKeySetError;
...
impl CoseKeySet {
    ...
    pub fn all_ecc_keys<V: EcdsaCoseKeyExt>(&self) -> impl Iterator<Item = &crate::CoseKey> {
        self.0
            .iter()
            .filter(|&k| k.alg() == Some(V::alg()) && k.crv() == Some(V::crv()))
    }

    pub fn find_keys(&self, alg: &iana::Algorithm) -> impl Iterator<Item = &crate::CoseKey> {
        self.0.iter().filter(move |&k| k.alg() == Some(*alg))
    }
}

impl std::ops::Deref for CoseKeySet {
    type Target = [crate::CoseKey];
...
pub struct CoseKeySetBuilder {
    keys: indexmap::IndexSet<crate::CoseKey>,
}
...
impl CoseKeySetBuilder {
    ...
    where
        ...
        <K as KeypairRef>::VerifyingKey: TryInto<crate::CoseKey, Error: Into<CoseKeySetError>>,
    {
        if !self.keys.insert(key.verifying_key().try_into().map_err(Into::into)?) {
    ...
    // fluent builder
    /// WARNING: do not put a private key in here !
    pub fn with_cose_key(mut self, key: crate::CoseKey) -> Result<Self, CoseKeySetError> {
        self.and_cose_key(key)?;
        Ok(self)
    }

    /// use this in iterators
    pub fn and_cose_key(&mut self, key: crate::CoseKey) -> Result<(), CoseKeySetError> {
        if cfg!(debug_assertions) && key.is_private() {
            panic!("Private keys should never be used in a CoseKeySet generally meant to be public");
        }
